...
#' @param boot Boolean indicating whether to return only a vector of statistics. The default is FALSE, which implies the object produced by nlminb2 will be returned
{
	...
	else
	{
		...
		else
		{
			...
			# Poids pour la matrice de l'ÃÂ©cole
			wte = rbind(apply(wj[c(2,4),],2,sum),apply(wj[c(6,8),],2,sum))
		}
	}

	# Création du vecteur de comptes y, du vecteur de poids w et du vecteur d'indices de dÃÂ©but de chaque matrice iniv
	y = w = NULL
...
for(k in 1:(nn-1))
{
	...
	for(h in 1:nrow(imat[[k]]))
	{
		...
		# Matrice des indices de la ligne des contacts de la tranche d'ÃÂ¢ge k pour les participants de la 1re tranche d'ÃÂ¢ge des participants
...
# On utilise la dernière entrée de imat donnant toutes les matrices qui peuvent être impliquées
		...
		for (l in 1:ncol(ml))
			# Pour la matrice ml des lignes dans les contraintes, la tranche d'âge k contribue seulement si les participants de cette tranche sont inclus dans la matrice impliquÃÂ©e
